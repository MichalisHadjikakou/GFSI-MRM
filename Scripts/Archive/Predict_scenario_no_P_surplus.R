...
ind <- c("Cropland","Pasture","Water","CH4","N2O", # Forest and LUC are derived post-hoc (on the basis of pasture and cropland)
         "Nfert","Nsurplus","Pfert") # Using 'Nfert' instead of "Nfix' as per Springmann et al. (2018) 
system <- c("Land-System Change","Land-System Change","Freshwater Use","Climate Change","Climate Change",
           "Biogeochemical Flows","Biogeochemical Flows","Biogeochemical Flows")
...
plan(multiprocess, workers = length(ind)) # Can change this accordingly (If 2010, use crop_num)
# 
...
#for (i in seq_along(ind)){
  ...
  Final_preds <- cbind(Ind_preds,Pred_int) %>%  # Selecting only relevant columns for summarising/presenting prediction results
    ...
    mutate(SD_lwr = Pred_avg-Pred_lwr,
           SD_upr = Pred_upr-Pred_avg,
  ...
  #   
     Results <- Final_preds %>% 
       add_column(Indicator=ind[i],.before=1) %>% # Adding indicator name
       add_column(System=system[i],.before=1) # Adding PB system name
...
## 2.2 Estimating risk by encompassing uncertainty from SD in model estimate and control variable triangular distribution
  ...
  write.csv(Results,paste0("Outputs/Meta-regression/Predictions/All_models_",today,"/",ind[i],"_",today,".csv"))
