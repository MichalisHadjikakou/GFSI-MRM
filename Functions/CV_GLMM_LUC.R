...
CV_GLMM <- function(stat_df,CV_reps,ind){

  k <- stat_df$Model %>% unique %>% length() -1 # Number of random intercepts minus 1 to allow sufficient reps
  ...
  fold_counts <- round(seq(from = 5, to = k)) %>% rep(each=CV_reps)
