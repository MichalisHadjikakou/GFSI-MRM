...
# Setting up loop through cut-off percentages

future::plan('multisession', workers = length(perc_cut)) 

future.apply::future_sapply(seq_along(perc_cut),function(x){
...
## Recoding df_likely (low and lowest risk scenarios) data.frame to improve visual representation
...
#Int_names_mod[Int_names=="N/P management"] = "N/P\nmanagement"
...
# ggsave(paste0("Risk_level_alluvial_",penalty,"SF_",scaling_factor,"_",ambition_cutoff,".tiff", sep = " ") , plot = p,
#        dpi = 600, width = 120, height = 200, units = "mm",device ="tiff",
#        compression="lzw", type="cairo")

# Combining plots
# cowplot::plot_grid(labels = "AUTO",
#                    p,summary_plot,
#                    nrow=1,ncol=2,
#                    rel_widths = c(1,0.75))
...
## Writing figures as .pdfs and results tables as .xlsx with muliple worksheets
...
write.csv(df_results,paste0("Intervention_ambition_by_risk_class_","Ambition_cut-off_",perc_cut[x]," %",".csv"))
       ...
       #compression="lzw", type="cairo")
# Demand-side parameters (normalised numbers)
# df_demand <- df_likely %>% 
#   count(Pop_levels,Diet,Plant_kcal,Waste,Likely)# %>% 
# mutate(n = case_when(
#   Likely=="33.3%>Risk<50%" ~ n/N_scen$Freq[1],
#   Likely=="Risk<33.3%" ~ n/N_scen$Freq[2],
#   Likely=="Risk>0.50" ~ n/N_scen$Freq[3]))

# df_demand <- df_likely_mod %>% 
#   count(Pop_levels,Diet,Plant_kcal,Waste,Likely) %>% 
#   filter(Likely!="Risk>0.50")
# 
# ggplot(df_demand, #%>% filter(Likely!="Risk>0.50"),
#        aes(y = n,
#            axis1 = factor(Pop_levels,levels=levels1), axis2 = Diet, axis3 = Plant_kcal, axis4 = Waste)) +
#   geom_alluvium(aes(fill = Likely),
#                 width = 0, knot.pos = 0, reverse = FALSE) +
#   #geom_alluvium()+
...
#   geom_text(stat = "stratum", aes(label = after_stat(stratum)),
#             reverse = FALSE) +
#   #scale_x_discrete(limits = c("Population","Animal calories","Plant calories","Waste"),
#   #                expand = c(0.15, 0.05, 0.05, 0.05)) + 
#   scale_x_continuous(breaks = 1:4, labels = c("Population", "Animal calories", "Plant calories","Waste")) +
...
# df_supply <- df_likely_mod %>% 
#   count(Yield_levels, Feed_efficiency, Feed_composition, WUEinc, Carbon_price, N_management, Likely) %>% 
#   filter(Likely!="Risk>0.50")
# 
# ggplot(df_supply, #%>% filter(Likely!="Risk>0.50"),
#        aes(y = n,
#            axis1 = factor(Yield_levels,levels=levels1), axis2 = Feed_efficiency, axis3 = Feed_composition, axis4 = WUEinc,
#            axis5 = Carbon_price, axis6 = N_management)) +
#   geom_alluvium(aes(fill = Likely),
#                 width = 0, knot.pos = 0, reverse = FALSE) +
#   #geom_alluvium()+
#   geom_stratum(width = 1/4, reverse = FALSE) +
...
#   #scale_x_discrete(limits = c("Population","Animal calories","Plant calories","Waste"),
#   #                expand = c(0.15, 0.05, 0.05, 0.05)) + 
#   scale_x_continuous(breaks = 1:6, labels = c("Crop yields", "Feed efficiency", "Feed composition","Water-use efficiency", "Carbon price", "N/P management")) +
#   #scale_fill_brewer(type = "qual", palette = 2) + 
#   scale_fill_manual(values = c("orange", "blue","white"))+
#   coord_flip() +
#   theme_bw() + 
...
# 
# ggplot(df_recoded,#df_recoded %>% filter(Likely=="Yes")
#        aes(y=1/sum(N_scen)*100,#y = 1/N_scen[2]*100,
#            axis1 = Pop_levels, axis2 = Diet, axis3 = Plant_kcal, axis4 = Waste, axis5 = Yield_levels, axis6 = Feed_efficiency, axis7 = WUEinc, 
#            axis8 = Carbon_price, axis9 = N_management)) +
#   geom_alluvium(aes(fill = Likely),
#                 width = 0, knot.pos = 0, reverse = FALSE) +
#   #geom_alluvium()+
#   guides(fill = FALSE) +
...
#   geom_text(stat = "stratum", aes(label = after_stat(stratum)),
#             reverse = FALSE) +
#   scale_x_continuous(breaks = 1:9, labels = c("Population", "Animal calories", "Plant calories","Waste","Crop yields","Feed efficiency",
#                                               "WUE", "Carbon price","N management")) +
#   scale_fill_brewer(type = "qual", palette = 2) + 
#   coord_flip() 
# 
# 
