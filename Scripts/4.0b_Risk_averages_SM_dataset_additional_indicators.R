...
# Outputs: Data for SM barplot Figure S4
...
sim_date <- "2024-08-04" # Date of simulation
...
for (k in seq_along(ind)){
  ...
  data <- fread(paste0("Outputs/Meta-regression/Predictions/All_models_",sim_date,"/",ind[k],"_",sim_date,".csv")) %>%  # Reading in csv with all generated predictions based on indicator model
    ...
    rename(any_of(c(Plant_kcal = "Vegetal_kcal_supply",
  ...
  Predictors[[k]] <- Col_names[1:grep("fit",Col_names)[1]-1] # Only selecting pure predictor names which varies with indicator
  ...
  list_all[[k]] <-  All_combs %>% 
    ...
    dplyr::select(Indicator,Predictor,Level,Pred_avg,SD_avg)
...
writexl::write_xlsx(All_results,paste0("Outputs/Composite_barplots/Figure_SM_additional_indicators_data_table_",
                                       today,".xlsx"))
