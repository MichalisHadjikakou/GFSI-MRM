...
# Outputs: Figure 3 in main manuscript
...
sim_date <- "2021-12-13" # Date of simulation - Change this depending on file being read
...
if(PC=='work_laptop') {
  setwd("N:/LES/Burwood/Brett-lab/Michalis/Future_food_systems_review/")
} else if (PC =='analytix_MH') {
  setwd("N:/Future_food_systems_review/")
} else if (PC =='analytix_BB') {
  setwd("N:/Brett-lab/Michalis/Future_food_systems_review/")
} else {
  setwd("//school-les-m.shares.deakin.edu.au/school-les-m/Planet-A/Food-Systems/Meta_analysis/")  
}
...
source("Scripts/All_variable_levels_bad_to_good_211201.R") # All levels for all indicators

All_pred_lists <- readRDS(paste0("Results/Composite_barplots/Figure_3_data_list_SI_",sim_date,".rds")) # Saving models for prediction

# Indicator names
ind <- c("Water","Nfert","Nsurplus","Pfert","Psurplus")

ind_names <- c("Blue water","N fertiliser","N surplus","P fertiliser","P surplus") # Revised indicator names 
...
All_levels[[which(All_preds=="Pop_levels")]] <- All_levels[[which(All_preds=="Pop_levels")]] %>% round(1) # Population simplified
All_levels[[which(All_preds=="Carbon_price")]] <- All_levels[[which(All_preds=="Carbon_price")]] %>% round(1) # Population simplified
...
All_levels_mod[[5]] <- c(expression(paste("15 %", Delta), 
                                    ...
                                    paste("60 %", Delta)))
# Feed efficiency
All_levels_mod[[6]] <- c(expression(paste("Current FE",", 0 %", Delta), 
                                    paste("Trend FE gap closure",", 12.5 %", Delta), 
                                    paste("Accelerated FE gap closure",", 25 %", Delta), 
                                    paste("Full FE gap closure",", 37.5 %", Delta)))
...
All_levels_mod[[8]] <- c(expression(paste("0/0 %", Delta, "CH4"[EI],"/N2O"[EI], ", $0 tCO2"[2]*"eq"), 
                                    paste("13/4 %", Delta, "CH4"[EI],"/N2O"[EI], ", $33 tCO2"[2]*"eq"),
                                    paste("27/8 %", Delta, "CH4"[EI],"/N2O"[EI], ", $66 tCO2"[2]*"eq"),
                                    paste("40/12 %", Delta, "CH4"[EI],"/N2O"[EI], ", $100 tCO2"[2]*"eq")))
...
All_levels_mod[[11]] <- c(expression(paste("0 %", Delta, "PUE", ", 0 %", Delta, "Recycling"), 
                                    paste("5 %", Delta, "PUE", ", 20 %", Delta, "Recycling"), 
                                    paste("10 %", Delta, "PUE", ", 40 %", Delta, "Recycling"), 
                                    paste("15 %", Delta, "PUE", ", 60 %", Delta, "Recycling")))
# # Nutrient recycling
...
png(filename = paste0('Results/Composite_barplots/Fig_3_Composite_barplot_SI_FWU+BF_',today,'.png'), width=wd, height=ht, units='in', res=600)
...
par(mai = c(0, 0, 0.3, 0),omi=c(0.15, 0.35, 0.35, 0.65)) # set margins mar=c(0.3, 0.25, 0, 0.25),oma=c(0.2,0,4,0.2),
...
ylimits <- c(-0.5,1) # Setting limits for y-axis
lw <- 0.5
cx <- 0.8
...
for (i in 1:level_num){
  ...
  for (j in 1:length(Pred_levels)){
    ...
    plot_data <- All_pred_lists[[i]] %>% filter(Level==as.character(Pred_levels[[j]])) %>%# dataset selection for each individual plot
                  ...
                  dplyr::select(Level,Pred_mean,Risk_joint_mean,Risk_joint_median,n,Risk_L,Risk_H,Risk_Q25,Risk_Q75,Indicator,Predictor) #%>% # Creating summarised dataframe of data to plot
                  #dplyr::mutate(across(c(Risk_joint_mean:Risk_Q75),~(.)*100)) # Turning into percentage change figures
      
    synth_data <- data.frame(Level= Pred_levels[j],Pred_mean=NA,Risk_joint_mean=NA,Risk_joint_median=NA,n=NA,Risk_L=NA,Risk_H=NA,Risk_Q25=NA,Risk_Q75=NA) %>% 
    ...
    synth_data$Colour <- c(brewer.pal(3,"Blues")[3],brewer.pal(7,"Purples")[6:3])
    ...
    if (j==2) {
      ...
      num_lab <- c(sprintf("%.*f", dp,Risk_abs[1:3]),sprintf("%.*f", 1,Risk_abs[4:5])) # Adding one decimal place for phosphorus
      num_lab[num_lab=="NA"] <- "" 
      ...
      non_na_risk <- !is.na(synth_data$Pred_mean) # determine positions of non-na numbers
      Risk_trend <- All_pred_lists[[i]] %>% 
        dplyr::filter(Level==Trend_levels[[i]]) %>%
        dplyr::filter(Indicator %in% ind) %>% 
        dplyr::arrange(factor(Indicator, levels = ind)) %>% 
        pull(Pred_mean) # extract risk for trend levels of predictor
      Risk_abs[non_na_risk] <- Risk_trend
      
      Risk_diff <- synth_data$Pred_mean - Risk_abs # This is risk difference
      num_lab <- c(sprintf("%+.*f", dp,Risk_diff[1:3]),sprintf("%+.*f", 1,Risk_diff[4:5])) # Adding one decimal place for phosphorus
      num_lab[num_lab=="NA"] <- "" 
    ...
    if (j==1){ # Axis only on first column (this can easily be arranged in loop)
      ...
      barplot(rep(NA,length(synth_data$Risk_joint_mean)), ylim=ylimits, axes=FALSE)
      axis(3, at=seq(-0.5, 15.5, by=1), lwd=lw, lwd.ticks=0, las=2, hadj=0.2, labels=FALSE, col="gray70")
      abline(h=0, lty=1, lwd=lw)
      #abline(h=1, col="gray70", lty=1, lwd=lw)
      # grid(nx=NA, ny=2, col="gray70", lty=1, lwd=lw)
      grouped_bar = barplot(synth_data$Risk_joint_mean, beside=T, ylim=ylimits, xaxt="n", axes = FALSE,add=TRUE,#panel.first = 
      ...
      axis(1, at=seq(-0.5, 15.5, by=1), lwd=lw, lwd.ticks=0, las=2, hadj=0.2, labels=FALSE, col="gray70")
      axis(2, at = seq(0, 1, by=0.5), lwd=lw, lwd.ticks=lw, las=2, tck=-0.05, cex.axis=cx-0.05, lab=seq(0, 1, by=0.5)*100, hadj=0.25)
      axis(2, at = seq(-1, -0.5, by=0.5), lwd=lw, lwd.ticks=lw, las=2, tck=-0.05, cex.axis=cx-0.05, lab=seq(-1, -0.5, by=0.5)*100, hadj=0.40)
      axis(2, at = seq(-0.5, 0, by=0.5),lwd.tick=0, labels=FALSE,cex.axis=cx-0.05,las=2,lwd=lw)
      #title(capFirst(tolower(Pred_levels[j])),line = 0.5)
      title(All_levels_mod[[i]][j], line=hpadj[i], cex.main=cx, font.main=1, xpd=T)
      text(as.vector(grouped_bar), ylimits[2]+0.15, srt = 0, adj= 0.45+text_offset, xpd = TRUE, col = "gray60", cex=cx-0.05,
           labels = num_lab)
      ...
      grouped_bar = barplot(synth_data$Risk_joint_mean, beside=T, ylim=ylimits,xaxt = "n",axes = FALSE, add=TRUE,#panel.first = 
                            ...
                            col=adjustcolor(synth_data$Colour,alpha_num),names.arg="",las=2,border=NA,cex.names=cx)
      # abline(h=0, col="black", lty=1, lwd=lw)
      axis(1, at=seq(-0.5, 15.5, by=1), lwd=lw, lwd.ticks=0, las=2, tck=-0.05, cex.axis=cx, hadj=0.2, labels=FALSE, col="gray70")
      title(All_levels_mod[[i]][j], line=hpadj[i], cex.main=cx, font.main=1, xpd = T)
      text(as.vector(grouped_bar), ylimits[2]+0.15, srt = 0, adj= 0.45+text_offset, xpd = TRUE, col = "gray60", cex=cx-0.05,
           labels = num_lab)
    }  
    ...
    # rect(xleft=as.vector(grouped_bar)-0.125, ybottom=Q25_lim, xright=as.vector(grouped_bar)+0.125, ytop=Q75_lim, lwd=lw, border="black", col="white")
    ...
    legend("bottomleft", title.col="black", bty="n", cex=cx*1.25, x.intersp=0.3, y.intersp=1.3, xpd=NA, legend=ind_names, 
           fill=adjustcolor(synth_data$Colour, alpha_num), ncol=length(ind), border="white", 
           inset=c(-3,-0.75),text.width=c(5,4.5,4,4,4)) 
    ...
    mtext("Waste\nreduction",            side=4, outer=T, at=tp-(gp*3),  line=ln, cex=cx, adj=aj, padj=pj, las=2, font=fnt)
    ...
    mtext("Percentage change relative to 2010 estimate", side=2, outer=T, at=0.51, line=1.4, cex=cx*1.25, padj=-0.2, font=1)

    mtext("Level of mitigation ambition", side=3, outer=T, at=0.49, line=1.4, cex=cx*1.25, font=1)
    mtext("Low",                          side=3, outer=T, at=0.119, line=0.3, cex=cx, font=fnt)
    mtext("Trend",                        side=3, outer=T, at=0.365, line=0.3, cex=cx, font=fnt)
    mtext("High",                         side=3, outer=T, at=0.618, line=0.3, cex=cx, font=fnt)
    mtext("Very High",                    side=3, outer=T, at=0.867, line=0.3, cex=cx, font=fnt)
