...
sim_date <- "2022-05-18" # Date of simulation
...
for (k in seq_along(ind)){
  ...
  data <- read.csv(paste0("Outputs/Risk_estimates/",ind[k],"_",sim_date,".csv")) %>%  # Reading in csv with all generated predictions based on indicator model
...
saveRDS(All_pred_lists,paste0("Outputs/Composite_barplots//Figure_1_data_list_",today,".rds")) # Saving models for prediction
writexl::write_xlsx(All_results,paste0("Outputs/Composite_barplots/Figure_1_data_table_",today,".xlsx"))
