##################################################
## FUTURE FOOD SCENARIOS - SM risk-compliant combinations ##
##################################################
...
# Outputs: Figures S6-S9 (using inputs from script 6.0a)
...
ind_names <- c("Agricultural_area","GHG_emissions","Surface_water_flows","Nutrient_cycles")
...
future::plan('multisession', workers = length(sel_cols)) 

future.apply::future_sapply(seq_along(sel_cols),function(x){

df_recoded <- All_scen %>% 
  mutate(Likely = case_when(
    !!rlang::sym(sel_cols[x]) <= threshold_high ~ "Risk<0.50",
    TRUE ~ "Risk>0.50")) %>% 
  mutate(Likely = case_when(
    !!rlang::sym(sel_cols[x]) <= threshold_low ~ "Risk<0.33",
    TRUE ~ Likely)) %>% 
  ...
  # 2-column
  ggsave(paste0("Outputs/Pareto_analysis+plots/Risk_level_summary_",ind_names[x],"_",Sys.Date(),".png") , plot = combined_unstretched,
