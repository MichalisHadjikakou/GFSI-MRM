...
DF_preprocess <- function(data_raw, PB_matrix){ # Reading in raw data file
  ...
  data_recoded <- data_raw %>% 
    dplyr::rename(Study =`Study abbreviation`,Scen_desc = `Scenario description`, ScenYear =`Scenario year (numeric)`,
           ...
           Aqua_kcal = Aquaculture, Ocean_kcal = `Capture seafood`, Seafood_kcal = `Total seafood`, 
           Dairy_eggs_kcal = `Dairy & eggs combined`,Dairy_kcal = Dairy, Eggs_kcal = Eggs,
           ...
           FCR_mon_meat = `Non-ruminant meat FCR`, FCR_monogastrics = `Non-ruminant FCR (meat & eggs)`,
           FCR_dairy = `Dairy FCR`, FCR_eggs = `Eggs FCR`,FCF_dairy = `FCF dairy`,FCF_eggs = `FCF egg`,
           FCF_ruminants = `FCF All ruminants`,FCF_monogastrics = `FCF All non-ruminants`,
  ...
  sel_cols <- c("DirNonCO2LUC","DirNonCO2","DirIndGHG","LUC","Aqua_kcal","Ocean_kcal","Seafood_kcal",
                "FCR_rum_meat","FCR_mon_meat","FCR_dairy","FCR_eggs","FCR_dairy_eggs",
                "FCR_ruminants","FCR_monogastrics","FCF_ruminants","FCF_monogastrics",
  ...
  DF <- DF %>% 
    ...
    mutate(Rum_meat_crop_feed_water = Population*(Rum_meat_kcal*FCR_FCF_rum_meat), # Composite variable with FCR+FCF  # Remove water eventually from name
           ...
           Eggs_feed_water = Population*(Eggs_kcal*FCR_FCF_eggs),# Composite variable with FCR+FCF
           ...
           All_feed_water = case_when(
             is.na(Rum_feed_water) ~ Mon_feed_water*2.00, # Adding up ruminant and monogastric feed with appropriate conversion to physical units
             is.na(Mon_feed_water) ~ Rum_feed_water,
             TRUE ~ Rum_feed_water + Mon_feed_water*2.00),
