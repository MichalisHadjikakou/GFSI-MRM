...
sim_date <- "2022-05-19" # Date of simulation - Change this depending on file being read
today <- Sys.Date() # Today's date
Base_year <- 2010
Scen_year <- 2050
levels <- 4
Risk_metric <- 'absolute' # Change to relative for +/- relative to trend
...
# Indicator names
ind <- c("Land_system_change","Climate_change","Freshwater_Use","Biogeochemical_Flows_N","Biogeochemical_Flows_P")
#"Nfert","Nsurplus","Pfert","Psurplus")

ind_names <- c("Land-system change","Climate change","Freshwater use","Biogeochemical Flows - N","Biogeochemical Flows - P") # Revised indicator names
...
for (i in 1:level_num){
  ...
  for (j in 1:length(Pred_levels)){
    ...
    synth_data$Colour <- c(brewer.pal(5,"Greens")[4],brewer.pal(8,"Oranges")[5],brewer.pal(3,"Blues")[3],brewer.pal(7,"Purples")[c(5,4)])
    ...
    legend("bottomleft", title.col="black", bty="n", cex=cx*1.25, x.intersp=0.3, y.intersp=1.3, xpd=NA, legend=ind_names, 
           ...
           inset=c(-3.3,-0.75),text.width=c(5,4.5,4,3.8,4.25)) 
    ...
    mtext("Risk of exceeding environmental limits", side=2, outer=T, at=0.51, line=1.4, cex=cx*1.25, padj=-0.2, font=1)
