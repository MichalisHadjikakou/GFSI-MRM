...
sim_date <- "2023-02-05" # Date of simulation - Change this depending on file being read
...
All_pred_lists <- readRDS(paste0("Outputs/Composite_barplots/Figure_SM_data_list_",sim_date,".rds")) # Saving models for prediction

# Indicator names
if (Boundaries=="LSC+CC"){
  ind <- c("Cropland","Pasture","CH4","N2O","CO2_LUC")

  ind_names <- c("Cropland","Pasture","Methane","Nitrous oxide","Land use change emissions") # Revised indicator names 

}else{
  
  ind <- c("Water","Nfert","Nsurplus","Pfert","Psurplus")
  
  ind_names <- c("Blue water","N fertiliser","N surplus","P fertiliser","P surplus") # Revised indicator names  # Revised indicator names 
}  
...
png(filename = paste0('Outputs/Composite_barplots/Fig_1_SM_Composite_barplot_',Boundaries,'_',Risk_metric,'_',today,'.png'), width=wd, height=ht, units='in', res=600)
...
if (Boundaries=="LSS+CC"){
  ylimits <- c(-1,0.75) # Setting limits for y-axis
}else{
  ylimits <- c(-0.5,0.75) # Setting limits for y-axis
}
...
for (i in 1:level_num){
  ...
  for (j in 1:length(Pred_levels)){
    ...
    plot_data <- All_pred_lists[[i]] %>% dplyr::filter(Level==as.character(Pred_levels[[j]])) %>%# dataset selection for each individual plot
    ...
    # Adjustment to cap high/low bar values 
    synth_data$Risk_joint_mean[synth_data$Risk_joint_mean>ylimits[2]] <- ylimits[2]
    #synth_data$Risk_joint_mean[synth_data$Risk_joint_mean<ylimits[1]] <- ylimits[1]
    
    if(Boundaries=="LSS+CC"){
      synth_data$Colour <- c(brewer.pal(5,"Greens")[3:4],brewer.pal(8,"Oranges")[4:6])#brewer.pal(3,"Blues")[3],brewer.pal(7,"Purples")[3:6])
    }else{
    ...
    if (Risk_metric=='absolute'){
      ...
      if (j==1){ # Axis only on first column (this can easily be arranged in loop)
        ...
        text(as.vector(grouped_bar), 0.9, srt = 0, adj= 0.45+text_offset, xpd = TRUE, col = "gray60", cex=cx-0.05,
             labels = num_lab)
        ...
        if (Risk_metric=='relative'& j==2){ # Different colour for Trend if risk metric is relative
          text(as.vector(grouped_bar), 0.9, srt = 0, adj= 0.45+text_offset, xpd = TRUE, col = "gray30", cex=cx-0.05,
               labels = num_lab)
        }else{
          text(as.vector(grouped_bar), 0.9, srt = 0, adj= 0.45+text_offset, xpd = TRUE, col = "gray60", cex=cx-0.05,
               labels = num_lab)  
    ...
    if(Boundaries=="LSS+CC"){
      ...
      legend("bottomleft", title.col="black", bty="n", cex=cx*1.25, x.intersp=0.3, y.intersp=1.3, xpd=NA, legend=ind_names, 
           ...
           inset=c(-3.2,-0.75),text.width=c(5,4.5,4,3.8,4)) 
    }else{
      legend("bottomleft", title.col="black", bty="n", cex=cx*1.25, x.intersp=0.3, y.intersp=1.3, xpd=NA, legend=ind_names, 
           fill=adjustcolor(synth_data$Colour, alpha_num), ncol=length(ind), border="white", 
