...
sim_date <- "2022-05-17"# Date of simulation
...
plan(multiprocess, workers = length(ind))
...
#for (k in seq_along(ind)){
  ...
  data <- read.csv(paste0("GLMMs/All_models_",sim_date,"/",ind[k],"_",sim_date,".csv")) %>% # Reading in csv with all generated predictions based on indicator model
...
saveRDS(All_pred_lists,paste0("Results/Composite_barplots/Figure_3_data_list_SI_",today,".rds")) # Saving models for prediction
writexl::write_xlsx(All_results,paste0("Results/Composite_barplots/Figure_3_data_table_SI_",today,".xlsx"))
