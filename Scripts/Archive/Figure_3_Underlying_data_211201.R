...
sim_date <- "2021-12-11" # Date of simulation
...
source("Scripts/All_variable_levels_bad_to_good_211201.R") # Need to add to script folder
...
for (k in seq_along(ind)){
  ...
  data <- read.csv(paste0("Results/GLMM_results/",ind[k],"_",sim_date,".csv")) # Reading in csv with all generated predictions based on indicator model
  ...
  for (i in seq_along(groups)) { # Looping through each predictor variable included 
    ...
    xtabs[[ind_preds[i]]] <- data %>% # Saving as dataframe in the right position
      ...
      summarise(n = dplyr::n(),
                ...
                Risk_joint_mean = mean(Risk,na.rm=TRUE), Risk_joint_median = median(Risk,na.rm=TRUE),
                Risk_Q25 = quantile(Risk,.25,na.rm=TRUE), Risk_Q75 = quantile(Risk,.75,na.rm=TRUE)) %>% 
...
saveRDS(All_pred_lists,paste0("Results/Composite_barplots/Figure_3_data_list_",today,".rds")) # Saving models for prediction
writexl::write_xlsx(All_results,paste0("Results/Composite_barplots/Figure_3_data_table_",today,".xlsx"))
